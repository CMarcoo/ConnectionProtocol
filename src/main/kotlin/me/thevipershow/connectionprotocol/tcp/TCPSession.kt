...
import me.thevipershow.connectionprotocol.event.session.PacketSendingEvent
import me.thevipershow.connectionprotocol.event.session.PacketSentEvent
import me.thevipershow.connectionprotocol.event.session.DisconnectedEvent
import me.thevipershow.connectionprotocol.event.session.DisconnectingEvent
import me.thevipershow.connectionprotocol.event.session.ConnectedEvent
import me.thevipershow.connectionprotocol.event.session.PacketReceivedEvent
...
abstract class TCPSession(private var host: String, private var port: Int, val protocol: PacketProtocol) :
    ...
    override fun send(packet: Packet) {
        if (this.channel != null) {
            val sendingEvent =
                PacketSendingEvent(this, packet)
            ...
            if (this.channel != null && this.channel!!.isOpen) {
                ...
                this.channel!!.flush().close().addListener(ChannelFutureListener {
                    run {
                        callEvent(
                            DisconnectedEvent(
                                reason ?: "Connection closed.", this@TCPSession, cause
                            )
                        )
                    }
                })
